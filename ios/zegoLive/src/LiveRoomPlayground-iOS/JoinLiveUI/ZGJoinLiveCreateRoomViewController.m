...
- (IBAction)onTryEnterRoom:(id)sender {
    ...
    [self gotoLivePageWithRoomID:roomID roomType:false];
}
...
- (IBAction)joinAgoraLiveRoom:(id)sender {
    NSString *srcRoomID = self.roomIDTxf.text;
     if (srcRoomID.length == 0) {
         return;
     }
    ...
    [self saveValue:srcRoomID forKey:ZGLoginRoomIDKey];
    [self gotoLivePageWithRoomID:roomID roomType:true];
    
    
}

- (void)gotoLivePageWithRoomID:(NSString *)roomID roomType:(bool)isAg {
    if (isAg) {
        UIStoryboard *sb = [UIStoryboard storyboardWithName:@"JoinLive" bundle:nil];
        AGJoinLiveViewController *liveVC = [sb instantiateViewControllerWithIdentifier:NSStringFromClass([AGJoinLiveViewController class])];
